var RadarChart = {
  draw: function(id, d, options){
    ...
    var radius = cfg.factor*Math.min((cfg.w-2)/2, (cfg.h-2)/2);
    d3.select(id).select("svg.radar").remove();
    var g = d3.select(id).append("svg").attr("class", "radar").attr("width", cfg.w+30).attr("height", cfg.h+30).append("g");
    ...
    for(var j=0; j<cfg.levels; j++){
      ...
      g.selectAll(".levels").data(allAxis).enter().append("svg:circle")
      	.attr('r', levelFactor)
      	.attr("class", "line").style("stroke", "#C9C9C9").style("fill", "none").style("stroke-width", "0.5px").attr("transform", "translate(" + (cfg.w/2) + ", " + (cfg.h/2) + ")");
      /*
    ...
    axis.append("text").attr("class", "legend")
        .text(function(d){return d;}).style("fill", "#1F77B4").style("font-family", "sans-serif").style("font-size", "10px").attr("transform", function(d, i){return "translate(-15, -5)"})
